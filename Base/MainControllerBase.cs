...
public class MainControllerBase<O, R>(IBaseService<O> service,
    IObjectMapper<O, R> mapper) : ControllerBase where O : class
{
    ...
    {
        var o = await service.FindByIdAsync(id);
        if (o == null)
        {
    ...
    protected async Task<ActionResult<List<R>>> AllAsync()
    {
        ...
        var elements = await service.AllAsync();
